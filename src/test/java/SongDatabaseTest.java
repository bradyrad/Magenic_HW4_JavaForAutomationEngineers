...
public class SongDatabaseTest {
    ...
    public void testAddDeleteSong() {
        ...
        Song song = new Song("Crack the Sky", "Mastodon");

        database.addSong(song);

        Assert.assertTrue(database.getSongArchive().contains(song));

        database.deleteSong(song);

        Assert.assertFalse(database.getSongArchive().contains(song));
    }
    ...
    public void testAddGetDeletePlaylist() {
        ...
        database.createPlaylist("New Playlist");

        Assert.assertTrue(database.getPlaylists().containsKey("New Playlist"));

        database.deletePlaylist("New Playlist");

        Assert.assertFalse(database.getPlaylists().containsKey("New Playlist"));
    }
